...
public extension LocalPublicKeyLoader {
    typealias Publisher = AnyPublisher<[String], Error>
...
public extension LocalSeedLoader {
    typealias Publisher = AnyPublisher<[String], Error>
...
extension Publisher {
    func caching(to cache: PublicKeyCache) -> AnyPublisher<Output, Failure> where Output == [String] {
        handleEvents(receiveOutput: cache.saveIgnoringResult).eraseToAnyPublisher()
    }
    
    func caching(to cache: PublicKeyCache) -> AnyPublisher<Output, Failure> where Output == Paginated<String> {
        handleEvents(receiveOutput: cache.saveIgnoringResult).eraseToAnyPublisher()
    }
}

private extension PublicKeyCache {
    func saveIgnoringResult(_ publicKeys: [String]) {
        try? save(publicKeys)
    }
    
    func saveIgnoringResult(_ page: Paginated<String>) {
