...
extension FailableRetrievePrivateKeyStoreSpecs where Self: XCTestCase {
    func assertThatRetrieveDeliversFailureOnRetrievalError(publicKey: String, on sut: KeychainPrivateKeyStore, file: StaticString = #filePath, line: UInt = #line) {
        expect(publicKey, sut, toRetrieve: .failure(anyNSError()), file: file, line: line)
    }
    
    func assertThatRetrieveHasNoSideEffectsOnFailure(publicKey: String, on sut: KeychainPrivateKeyStore, file: StaticString = #filePath, line: UInt = #line) {
