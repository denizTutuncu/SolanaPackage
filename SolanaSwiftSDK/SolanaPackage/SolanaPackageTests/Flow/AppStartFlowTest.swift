...
class AppStartFlowTest: XCTestCase {
    ...
    func test_start_withNoWallets_doesNotDelegateWalletHandling() {
    ...
    func test_start_withAtLeastOneWallet_delegatesCorrectWalletHandling() {
        makeSUT(wallets: ["First Wallet"]).start()
    ...
    func test_start_withOneWallet_delegatesAnotherCorrectWalletHandling() {
        makeSUT(wallets: ["Second Wallet"]).start()
        
        XCTAssertEqual(delegate.passedWallets, ["Second Wallet"])
    }

    func test_start_withTwoWallets_delegatesFirstWalletHandling() {
        makeSUT(wallets: ["First Wallet", "Second Wallet"]).start()
    ...
    private func makeSUT(wallets: [String]) -> AppStartFlow<WalletDelegateSpy> {
        let sut = AppStartFlow(delegate: delegate, wallets: wallets)
        weakSUT = sut
        return sut
    }
